...
def optimize_catboost_hyperparams(x_train, y_train, x_val, y_val, num_classes):
    """使用贝叶斯优化调优CatBoost超参数"""

    def catboost_hyper_param(learning_rate, depth, l2_leaf_reg, border_count):
    ...
    print("CatBoost最佳超参数:", best_params)
...
def train_transformer_model_fixed(x_train, y_train, x_val, y_val, num_classes):
    """使用固定合理参数训练Transformer模型（不再贝叶斯优化）"""
...
def calculate_metrics(catboost_model, transformer_model, x_test, y_test, label_encoder):
    """计算融合模型的评估指标（CatBoost 0.7 + Transformer 0.3）"""
    print("\n计算评估指标（CatBoost 0.7 + Transformer 0.3 融合）...")
    ...
    with torch.no_grad():
        ...
        transformer_logits = transformer_model(test_tensor.unsqueeze(1))
    ...
    for k in range(1, 6):
        ...
        for i in range(len(y_test)):
            top_k_indices = np.argsort(fused_proba[i])[-k:]
    ...
    return metrics_dict, fused_proba, y_pred, topk_accuracies


def create_visualizations(y_test, y_pred, y_pred_proba, label_encoder, topk_accuracies, output_folder, catboost_model,
                          transformer_model, feature_columns):
    """创建所有可视化图表"""
    print("\n生成可视化图表...")
    ...
    # 5. 特征重要性图（CatBoost特征重要性）
    catboost_importance = catboost_model.get_feature_importance()
    indices = np.argsort(catboost_importance)[::-1]
    plt.figure(figsize=(10, 6))
    plt.title("CatBoost特征重要性（融合模型）")
    plt.bar(range(len(catboost_importance)), catboost_importance[indices], align="center")
    plt.xticks(range(len(catboost_importance)), [feature_columns[i] for i in indices], rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_folder}/feature_importance.png', dpi=300, bbox_inches='tight')
    plt.close()


def save_results_and_model(catboost_model, transformer_model, label_encoder, metrics_dict, y_test, y_pred_proba,
                           output_folder):
    """保存融合模型和结果"""
    print("\n保存结果和模型...")
    ...
    # 保存两个模型
    joblib.dump(catboost_model, f'{output_folder}/catboost_model.pkl')
...
def create_usage_example(output_folder):
    """创建使用示例（融合模型）"""
    usage_code = f'''# 作物推荐系统使用示例（CatBoost 0.7 + Transformer 0.3 融合）
import joblib
import torch
import numpy as np
import torch.nn as nn
...
# 加载模型和编码器
catboost_model = joblib.load('{output_folder}/catboost_model.pkl')
...
def main():
    print("=" * 60)
    print("CatBoost + Transformer 融合作物推荐系统（CatBoost贝叶斯优化 + 固定Transformer）")
    print("=" * 60)
    ...
    # 仅对CatBoost进行贝叶斯优化
    print("\n开始CatBoost贝叶斯优化...")
    catboost_model = optimize_catboost_hyperparams(x_train, y_train, x_val, y_val, num_classes)

    # 使用固定参数训练Transformer
    print("\n训练固定参数的Transformer模型...")
    transformer_model = train_transformer_model_fixed(x_train, y_train, x_val, y_val, num_classes)

    # 融合评估（CatBoost 0.7 + Transformer 0.3）
    metrics_dict, y_pred_proba, y_pred, topk_accuracies = calculate_metrics(
        catboost_model, transformer_model, x_test, y_test, label_encoder)

    # 可视化
    create_visualizations(y_test, y_pred, y_pred_proba, label_encoder, topk_accuracies, output_folder, catboost_model,
                          transformer_model, feature_columns)

    # 保存
    save_results_and_model(catboost_model, transformer_model, label_encoder, metrics_dict, y_test, y_pred_proba,
                           output_folder)

    # 打印摘要
    print("\n" + "=" * 60)
    print("模型评估结果摘要（CatBoost 0.7 + Transformer 0.3 融合）")
    print("=" * 60)
    ...
    print("生成的文件列表")
    print("=" * 60)
    print(f"1. {output_folder}/catboost_model.pkl - CatBoost模型")
    print(f"2. {output_folder}/transformer_model.pth - Transformer模型")
    print(f"3. {output_folder}/label_encoder.pkl - 标签编码器")
    print(f"4. {output_folder}/evaluation_metrics.json - 评估指标")
    print(f"5. {output_folder}/detailed_predictions.csv - 详细预测（含0-100评分）")
    print(f"6. {output_folder}/usage_example.py - 使用示例")
    print(f"7. {output_folder}/topk_accuracy_curve.png")
    print(f"8. {output_folder}/normalized_confusion_matrix.png")
    print(f"9. {output_folder}/f1_per_class.png")
    print(f"10. {output_folder}/macro_pr_curve.png")
    print(f"11. {output_folder}/feature_importance.png")
